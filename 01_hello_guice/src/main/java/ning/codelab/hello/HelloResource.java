 ...
 * service delegates to the message configured by the configuration object. In
 * the future, more complex logic may be enabled.
 */
public class HelloResource {
	...
	public String getMessage() {
		// The currentTime is based on the property xn.current.time if not
		// specified, current systemtime will be used.

		DateTime currentTime = config.getCurrentTime();

		String dateFormat = fmt.print(currentTime);

		return config.getMessage() + getWishMessage(currentTime.getHourOfDay())
				+ " @ " + dateFormat;
	}
